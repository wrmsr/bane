...
import (
	...
	"github.com/wrmsr/bane/pkg/utils/optional"
	"github.com/wrmsr/bane/pkg/utils/slices"
)

func c3Merge[T comparable](seqs [][]T) ([]T, error) {
	// Merges MROs in *seqs* to a single MRO using the C3 algorithm.
	// Adapted from http://www.python.org/download/releases/2.3/mro/.

	var result []T
	var candidate optional.Optional[T]
	for {
		var nseqs [][]T
		for _, s := range seqs {
			if len(s) > 0 {
				nseqs = append(nseqs, s)
			}
		}
		seqs = nseqs
		...
		for _, s1 := range seqs { // find merge candidates among seq heads
			candidate = optional.Just(s1[0])
			for _, s2 := range seqs {
				if slices.Contains(s2[1:], candidate.Value()) {
					candidate = optional.None[T]()
					break
				}
			}
		...
		if !candidate.Present() {
			return nil, errors.New("inconsistent hierarchy")
		}

		result = append(result, candidate.Value())

		for i, seq := range seqs {
			if seq[0] == candidate.Value() {
				seqs[i] = slices.DeleteAt(seq, 0)
			}
		}
	}
}

//func mro[T comparable](
//	cls T,
//	abcs []T,
//	getBases func(T) []T,
//	isSubclass func(T, T) bool,
//) []T {
//	// Computes the method resolution order using extended C3 linearization.
//	//
//	// If no *abcs* are given, the algorithm works exactly like the built-in C3 linearization used for method
//	// resolution.
//	//
//	// If given, *abcs* is a list of abstract base classes that should be inserted into the resulting MRO. Unrelated
//	// ABCs are ignored and don't end up in the result. The algorithm inserts ABCs where their functionality is
//	// introduced, i.e. issubclass(cls, abc) returns True for the class itself but returns False for all its direct base
//	// classes. Implicit ABCs for a given class (either registered or inferred from the presence of a special method
//	// like __len__) are inserted directly after the last ABC explicitly listed in the MRO of said class. If two
//	// implicit ABCs end up next to each other in the resulting MRO, their ordering depends on the order of types in
//	// *abcs*.
//	for i, base := range getBases(cls) {
//
//	}
//}
//
