...
/*
In these tests, the §, ¶ and ∑ characters in readTest.Input are used to denote the start of a field, a record boundary
and the position of an error respectively. They are removed before parsing and are used to verify the position
...
`,
	...
	// The implementation may read each line in several chunks if it doesn't fit entirely in the read buffer, so we
	// should test the code to handle that condition.
	Name:    "HugeLines",
