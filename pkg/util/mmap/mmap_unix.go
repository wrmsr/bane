...
//
// The runtime.SetFinalizer documentation says that, "The finalizer for x is scheduled to run at some arbitrary time
// after x becomes unreachable. There is no guarantee that finalizers will run before a program exits", so we cannot
// automatically test that the finalizer runs. Instead, set this to true when running the manual test.
const debug = false
...
func Open(filename string) (*OpenReaderAt, error) {
	...
	size := fi.Size()
	if size == 0 {
		// Treat (size == 0) as a special case, avoiding the syscall, since "man 2 mmap" says "the length... must be
		// greater than 0".
		//
		// As we do not call syscall.Mmap, there is no need to call runtime.SetFinalizer to enforce a balancing
		// syscall.Munmap.
		return &OpenReaderAt{
			data: make([]byte, 0),
		}, nil
	...
	runtime.SetFinalizer(r, (*OpenReaderAt).Close)
	return r, nil
}
