...
import (
	...
	binu "github.com/wrmsr/bane/core/binary"
	eu "github.com/wrmsr/bane/core/errors"
	iou "github.com/wrmsr/bane/core/io"
	mmapu "github.com/wrmsr/bane/core/mmap"
)
...
func machoForEachSym2(sv SliceView[byte], fn func(s FileSym) bool) (ret bool, err error) {
	...
	if err := binu.Read(&iou.ReaderAtReader{R: r}, f.ByteOrder, &f.FileHeader); err != nil {
		return false, err
	}
	...
	for i := uint32(0); i < f.Ncmd; i++ {
		...
		case macho.LoadCmdSymtab:
			...
			if err := binu.Read(b, bo, &hdr); err != nil {
				return false, err
			}
			...
			for i := 0; i < int(hdr.Nsyms); i++ {
				...
				if f.Magic == macho.Magic64 {
					// FIXME: allocs []byte
					if err := binu.Read(b2, bo, &n); err != nil {
						return false, err
					}
				} else {
					var n32 macho.Nlist32
					// FIXME: allocs []byte
					if err := binu.Read(b2, bo, &n32); err != nil {
						return false, err
					}
					n.Name = n32.Name
					n.Type = n32.Type
